...
export const BoardModal = ({bldgNo, posts, activePage, onPaginationClick, onWrite}) => {
  const handleBack = () => {
  ...
  if(posts && posts.length!==0){
    ...
    footer = (
      <Modal.Footer>
        ...
        <div className="col-5">
          <Button className="writePost float-right" variant="secondary" onClick = {()=>{onWrite(bldgNo)}}>쓰기</Button>
        </div>
      </Modal.Footer>
    )
  } else if(posts) {
